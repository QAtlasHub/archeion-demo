...
<style>
  ...
  .pinax-checks .pinax-codelink{color:#57606a;font-size:11px;text-decoration:none;border-bottom:1px dotted #b6bec7}
  .pinax-checks .pinax-codelink:hover{color:#0969da;border-bottom-color:#0969da}
  ...
  .pinax-code-src{background:#f6f8fa;border:1px solid #e2e5e9}
  .pinax-code-out{background:#fbfbfd;border:1px solid #eef0f2;border-top:none;color:#3d4451}
  .pinax-code-out::before{content:"⇒ ";color:#8a949e}
  ...
  .pinax-stat .stat-label{display:block;font-size:.8rem;color:#57606a}
...
function emit_check(::GalleryBase, chk, ctx)
    ...
    # The code that produced this check is ONE artifact in the section below (a `@code` block), so the
    # table — the verdict-and-margin view — links to it instead of carrying a second copy of it.
    isempty(chk.code_anchor) || print(
        io,
        " <a class=\"pinax-codelink\" href=\"#",
        _esc(chk.code_anchor),
        "\" title=\"the code that produced this check\">code</a>",
    )
    print(io, "</td>")
...
# A `@code` block: the source in a highlighted `<pre>`, then its captured output (if any).
function emit_code(::GalleryBase, blk, ctx)
    io = ctx.io
    println(io, "<div class=\"pinax-code\" id=\"", blk.anchor, "\">")
    isempty(blk.caption) ||
        println(io, "<div class=\"pinax-code-cap\">", _esc(blk.caption), "</div>")
    println(
        io,
        "<pre class=\"pinax-code-src\"><code class=\"language-",
        _esc(blk.lang),
        "\">",
        _esc(blk.source),
        "</code></pre>",
    )
    isempty(blk.output) || println(
        io, "<pre class=\"pinax-code-out\"><code>", _esc(blk.output), "</code></pre>"
    )
    println(io, "</div>")
    return nothing
end
