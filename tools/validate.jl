...
#
#     julia tools/validate.jl [root]
#
# Standard library only, so that it runs wherever Julia does. Exits 1 when any error is found;
# warnings are printed and do not fail. Section numbers below refer to SPEC.md.
...
const RESERVED = Set(["con", "prn", "aux", "nul",
                      ("com$i" for i in 1:9)..., ("lpt$i" for i in 1:9)...])   # R4
...
function zone_less_times(text)
    unquoted = replace(text, r"\"\"\"[\s\S]*?\"\"\"" => "", r"'''[\s\S]*?'''" => "",
                       r"\"(?:[^\"\\\n]|\\.)*\"" => "", r"'[^'\n]*'" => "", r"#[^\n]*" => "")
...
function load(r::Report, path)
    ...
    for t in zone_less_times(text)
        err!(r, path, "time `$t` has no `Z`; times inside files are UTC with a Z suffix (R7)")
    end
    try
...
getpath(d, keys...) = foldl((x, k) -> x isa AbstractDict ? get(x, k, nothing) : nothing,
                            keys; init = d)
...
function require(r, path, d, keys...; type = Any)
    ...
    v isa type || (err!(r, path, "`$name` must be a $(type), got $(typeof(v))"); return nothing)
    return v
end

check_spec(r, path, d) = get(d, "spec", nothing) == SPEC || err!(r, path, "`spec` must be \"$SPEC\"")
    ...
    for top in ("projects", "records")
        ...
        for (dir, dirs, files) in walkdir(base)
            ...
            for name in vcat(dirs, files)
                ...
                other == name || err!(r, joinpath(dir, name),
                    "differs from `$other` only in case; a case-insensitive filesystem merges them (R1)")
            end
            for name in vcat(dirs, files)
                path = joinpath(dir, name)
                occursin(':', name) && err!(r, path, "`:` in a path (R2)")
                occursin(SEGMENT, name) ||
                    err!(r, path, "segment `$name` uses characters outside ASCII [A-Za-z0-9_.-] (R1)")
                length(name) <= 64 || err!(r, path, "segment longer than 64 characters (R3)")
                length(rel(r, path)) <= 180 || err!(r, path, "path longer than 180 characters (R3)")
...
function check_projects(r::Report)
    ...
    for f in sort(readdir(base))
        ...
        if id !== nothing
            occursin(PROJECT_ID, id) || err!(r, path, "`id` is not a project identifier (R5)")
...
function check_values(r, path, x, where = "")
    if x isa AbstractDict
        for (k, v) in x
            ...
            k in FORBIDDEN_KEYS &&
                err!(r, path, "`$key`: a revision states what it used, not what is computed (§8)")
            if k == "commit"
                v isa String && occursin(r"^[0-9a-f]{40}$", v) ||
                    err!(r, path, "`$key` must be 40 hexadecimal characters")
            elseif endswith(k, "digest")
                v isa String && occursin(r"^sha256:[0-9a-f]{64}$", v) ||
                    err!(r, path, "`$key` must be \"sha256:\" and 64 hexadecimal characters")
            elseif k == "bytes"
                v isa Integer && v > 0 ||
                    err!(r, path, "`$key` must be a positive integer (omit it when unknown)")
            end
...
function check_sums(r::Report, revdir)
    ...
    isfile(sums) || (err!(r, revdir, "no SHA256SUMS: the revision is incomplete (§5.2)"); return)
    listed = Set{String}()
    ...
    for (dir, _, files) in walkdir(revdir), f in files
        ...
        file == "SHA256SUMS" || file in listed ||
...
function check_revision(r::Report, revdir, record, revs)
    ...
    m === nothing &&
        (err!(r, revdir, "revision directory is not `<YYYYMMDDTHHMMSSZ>-<tag>`"); return nothing)
    ...
    check_spec(r, path, e)
    for (k, want) in (("project", get(record, "project", nothing)),
                      ("record", get(record, "id", nothing)),
                      ("rev", name), ("kind", get(record, "kind", nothing)))
    ...
    frozen === nothing || frozen == DateTime(m[1], PATH_TIME) ||
    ...
    status === nothing || status in ("trial", "final") ||
    ...
    if stable !== nothing && loc !== nothing
        ...
        isempty(both) ||
            err!(r, path, "anchors both stable and local: $(join(sort(collect(both)), ", "))")
    end
    level = require(r, path, e, "preservation", "level"; type = String)
    level === nothing || level == "read" ||
        err!(r, path, "`preservation.level` is \"read\"; higher levels are earned by events (§6)")
    src = getpath(e, "source")
    if src !== nothing
        getpath(src, "captured") in ("run-start", "completion", "publish") ||
            err!(r, path, "`source.captured` must be \"run-start\", \"completion\" or \"publish\"")
    ...
    return (; name, parents = something(parents, []),
...
function check_events(r::Report, recdir, recid, revinfo)
    ...
    for f in sort(readdir(evdir))
        ...
        occursin(EVENT_FILE, f) ||
            (err!(r, path, "event file is not `<YYYYMMDDTHHMMSSZ>-<origin>-<key>.toml`"); continue)
        ev = load(r, path)
        ev === nothing && continue
        check_spec(r, path, ev)
        kind = require(r, path, ev, "kind"; type = String)
        kind === nothing || kind in EVENT_KINDS ||
        ...
        subj === nothing || subj == recid || err!(r, path, "`subject.record` $subj is not this record")
        ...
        elseif rev !== nothing && anchor !== nothing
            ...
            anchor in a.stable || warn!(r, path, anchor in a.loc ?
...
function current(revinfo, events)
    targets(k) = Set(getpath(ev, "subject", "rev") for ev in events if get(ev, "kind", "") == k)
...
function check_record(r::Report, recdir, year, projects, seen)
    ...
    m === nothing &&
        (err!(r, recdir, "record directory is not `<YYYY-MM-DD>-<slug>-<record-id>`"); return nothing)
    startswith(m[1], year) || err!(r, recdir, "created in $(m[1][1:4]) but filed under $year")
    id = String(m[3])
    ...
    rid === nothing || rid == id || err!(r, path, "`id` $rid does not match the directory's $id")
    kind = require(r, path, d, "kind"; type = String)
    kind === nothing || kind == "report" || err!(r, path, "`kind` must be \"report\" in registry/1")
    proj = require(r, path, d, "project"; type = String)
    proj === nothing || proj in projects || err!(r, path, "project $proj is not in projects/")
    created = require(r, path, d, "created"; type = DateTime)
    created === nothing || Dates.format(created, "yyyy-mm-dd") == m[1] ||
        err!(r, path, "created on $(Date(created)) but the directory says $(m[1])")
    revroot = joinpath(recdir, "revisions")
    names = isdir(revroot) ? sort(filter(n -> isdir(joinpath(revroot, n)), readdir(revroot))) :
            String[]
    ...
    state = length(heads) == 1 ? "current $(only(heads))" :
            isempty(heads) ? "withdrawn" : "in conflict: $(join(heads, ", "))"
...
function validate(root)
    ...
    for year in (isdir(base) ? sort(readdir(base)) : String[])
        ...
        isdir(ydir) && occursin(r"^\d{4}$", year) || (err!(r, ydir, "not a year directory"); continue)
...
if abspath(PROGRAM_FILE) == @__FILE__
    r, summary = validate(isempty(ARGS) ? pwd() : ARGS[1])
    foreach(s -> println("  ", s), summary)
    foreach(w -> println("warning: ", w), r.warnings)
    foreach(e -> println("error: ", e), r.errors)
    println(isempty(r.errors) ? "ok: $(length(summary)) record(s)" : "$(length(r.errors)) error(s)")
    exit(isempty(r.errors) ? 0 : 1)
end
